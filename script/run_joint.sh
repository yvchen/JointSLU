...
if [ ! $1 ]; then
	...
	MDL=$1
	TRAIN=$DATADIR/atis-2.train.w-intent.iob
	DEV=$DATADIR/atis-2.dev.w-intent.iob
	TEST=$DATADIR/atis.test.w-intent.iob
	TLEN=48
	VDIM=100
	...
	do
		...
		do
			if [ ! -d experiment/res.w-intent/"$HDIM"-"$DROPOUT" ]; then
				mkdir -p experiment/res.w-intent/"$HDIM"-"$DROPOUT"
			fi
			if [ ! -d experiment/mdl.w-intent/"$HDIM"-"$DROPOUT" ]; then
				mkdir -p experiment/mdl.w-intent/"$HDIM"-"$DROPOUT"
			fi
